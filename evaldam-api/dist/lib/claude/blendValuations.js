...
exports.blendValuations = blendValuations;
/**
 ...
 */
function blendValuations(profile, methods) {
    ...
    const qualitativeMethods = methods.filter((m) => m.methodName === "scorecard" || m.methodName === "berkus");
    const quantitativeMethods = methods.filter((m) => m.methodName === "vc" || m.methodName === "dcf-ltg" || m.methodName === "dcf-multiples");
    ...
    const weightedAverage = qualitativeAvg * qualitativeWeight + quantitativeAvg * quantitativeWeight;
    ...
    methods.forEach((method) => {
        const isQualitative = method.methodName === "scorecard" || method.methodName === "berkus";
...
function generateKeyReasons(profile, methods, valuation) {
    const reasons = [];
    // Reason 1: Stage + Growth
    if (profile.monthlyGrowthRate && profile.monthlyGrowthRate > 10) {
        reasons.push(`Strong monthly growth of ${profile.monthlyGrowthRate.toFixed(1)}% MoM indicates rapid scaling potential`);
    }
    else if (profile.annualRecurringRevenue && profile.annualRecurringRevenue > 100000) {
        reasons.push(`Existing revenue of $${(profile.annualRecurringRevenue / 1000000).toFixed(1)}M ARR provides strong traction`);
    }
    else {
        reasons.push(`${profile.stage === "pre-revenue" ? "Pre-revenue" : "Early-stage"} company with clear market opportunity`);
    }
    // Reason 2: Team/Achievements
    if (profile.team && profile.team.length > 2) {
        reasons.push(`Experienced team of ${profile.team.length} with relevant domain expertise`);
    }
    if (profile.accelerators &&
        profile.accelerators.some((a) => a.name === "YC" || a.name === "Techstars")) {
        reasons.push(`${profile.accelerators.find((a) => a.name === "YC" || a.name === "Techstars")?.name || "Top"}-backed company with credibility and network access`);
    }
    // Reason 3: Market
    if (profile.totalAddressableMarket && profile.totalAddressableMarket > 1000000000) {
        reasons.push(`Large addressable market of $${(profile.totalAddressableMarket / 1000000000).toFixed(1)}B+ with multiple expansion opportunities`);
    }
    // Reason 4: Multiple convergence
    const highMethod = methods.reduce((prev, current) => current.midEstimate > prev.midEstimate ? current : prev);
    const lowMethod = methods.reduce((prev, current) => current.midEstimate < prev.midEstimate ? current : prev);
    if (highMethod.midEstimate / lowMethod.midEstimate < 2) {
        reasons.push(`Multiple valuation methods converge on similar range, suggesting consistent valuation`);
    }
    else {
        reasons.push(`Range reflects methodological diversity; market conditions may drive to upper or lower bound`);
    }
    return reasons.slice(0, 4); // Return top 3-4 reasons
}
function generateSensitivityAnalysis(baseValuation) {
    return [
        {
...
//# sourceMappingURL=blendValuations.js.map