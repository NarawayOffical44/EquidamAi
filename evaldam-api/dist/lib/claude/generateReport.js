...
exports.generateOnePagerSummary = generateOnePagerSummary;
...
async function generateFullReport(profile, valuation) {
...
Generate each section separately but ensure continuity and professional flow.`;
    ...
    const executiveSummaryMatch = reportText.match(/## Executive Summary([\s\S]*?)(?=## |$)/i);
    const methodologyMatch = reportText.match(/## Valuation Methodology([\s\S]*?)(?=## |$)/i);
    const report = {
...
function formatAssumptions(valuation) {
...
function formatAppendix(valuation) {
...
async function generateOnePagerSummary(profile, valuation) {
