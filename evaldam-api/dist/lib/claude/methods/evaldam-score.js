...
class EvalDamScoreMethod extends base_method_1.ValuationMethodBase {
    constructor(profile) {
    ...
    async execute() {
        ...
        return {
            ...
            reasoning: this.buildDetailedReasoning(profile, internalPercentile, industryGrowthPremium, teamExitBonus, patentBonus, customerConcentrationRisk, marketTimingScore, moatStrength, investorCriteria, baseValuation, adjustedValuation),
            assumptions: {
     ...
     */
    calculateInternalPercentile(profile) {
        ...
        if (profile.teamExperienceYears) {
            const teamBench = benchmarks['team_experience'];
        ...
        if (profile.runwayMonths) {
            const expectedRunway = benchmarks['runway_months'];
     ...
     */
    calculateIndustryGrowthPremium(profile) {
     ...
     */
    calculateTeamExitBonus(profile) {
        ...
        if (totalExitValue > 100e6)
            return 0.15;
        if (totalExitValue > 10e6)
            return 0.10;
        if (totalExitValue > 0)
            return 0.05;
        return 0;
    ...
    calculateConcentrationRisk(profile) {
        ...
        if (concentration < 30)
            return 0; // Safe
        if (concentration < 50)
            return 0.05; // Mild risk: -5%
        if (concentration < 70)
            return 0.15; // High risk: -15%
     ...
     */
    calculateMarketTimingScore(profile) {
        let score = 50; // baseline
     ...
     */
    async assessMoatStrength(profile) {
...
`;
        try {
            const response = await (0, providers_1.callLLM)([{ role: 'user', content: prompt }], { useCase: 'valuation', temperature: 0.2, maxTokens: 300 });
    ...
    parseInvestorCriteria(profile) {
        ...
        if (context.includes('enterprise') || context.includes('b2b'))
            criteria['enterprise_focus'] = 20;
        if (context.includes('consumer') || context.includes('b2c'))
            criteria['consumer_focus'] = 20;
        if (context.includes('international') || context.includes('global'))
            criteria['global_reach'] = 15;
        if (context.includes('strategic'))
            criteria['strategic_value'] = 25;
        if (context.includes('regulatory'))
            criteria['regulatory_compliance'] = 10;
        return criteria;
    }
    /**
     * Determine confidence level based on data completeness
     */
    determineConfidence(profile) {
        let dataPoints = 0;
        if (profile.team?.length)
            dataPoints++;
        if (profile.annualRecurringRevenue)
            dataPoints++;
        if (profile.monthlyGrowthRate)
            dataPoints++;
        if (profile.customerCount)
            dataPoints++;
        if (profile.totalAddressableMarket)
            dataPoints++;
        if (profile.teamPreviousExits)
            dataPoints++;
        if (profile.patentCount)
            dataPoints++;
     ...
     */
    buildDetailedReasoning(profile, percentile, growthPremium, teamBonus, patentBonus, concentrationRisk, marketTiming, moatScore, investorCriteria, baseVal, finalVal) {
        const sections = [];
...
exports.EvalDamScoreMethod = EvalDamScoreMethod;
exports.default = EvalDamScoreMethod;
//# sourceMappingURL=evaldam-score.js.map