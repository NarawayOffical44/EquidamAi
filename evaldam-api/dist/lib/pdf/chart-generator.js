...
exports.generateValuationRangeChart = generateValuationRangeChart;
exports.generateMethodContributionChart = generateMethodContributionChart;
exports.generateSensitivityChart = generateSensitivityChart;
exports.generateMarketBenchmarksChart = generateMarketBenchmarksChart;
exports.generateValuationTrendsChart = generateValuationTrendsChart;
/**
 * Valuation Range Chart (Horizontal bar)
 */
function generateValuationRangeChart(lowEst, mid, highEst, companyName) {
    const width = 500;
    const barHeight = 40;
    const margin = 50;
    // Scale values
 ...
 */
function generateMethodContributionChart(methods) {
    const radius = 80;
    const centerX = 120;
    const centerY = 120;
    // Colors for methods
    ...
    <svg width="400" height="300" style="margin: 20px 0;">
      ...
      <!-- Pie chart -->
      ${segments
        .map((seg, idx) => `<path d="${seg.arc}" fill="${seg.color}" stroke="white" stroke-width="2"/>`)
        .join('')}
 ...
 */
function generateSensitivityChart(baseValuation, sensitivities) {
    const width = 500;
    const height = 250;
    const barWidth = 35;
    const margin = 60;
 ...
 */
function generateMarketBenchmarksChart(industry, arrMultiple) {
 ...
 */
function generateValuationTrendsChart() {
    ...
    <svg width="${width}" height="${height}" style="margin: 20px 0;">
      ...
      <!-- Points -->
      ${points
        .map((p, idx) => `<circle cx="${p.x}" cy="${p.y}" r="${pointRadius}" fill="#667eea"/>
      ...
      <!-- X-axis labels -->
      ${points
        .map((p, idx) => `<text x="${p.x}" y="${height - margin + 20}" font-size="11" text-anchor="middle" fill="#999">
         ${years[idx]}
       </text>`)
        .join('')}
