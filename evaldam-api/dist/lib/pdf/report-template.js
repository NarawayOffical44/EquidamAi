...
exports.generateProfessionalReportHTML = generateProfessionalReportHTML;
...
function generateProfessionalReportHTML(data) {
    ...
    // Method weights
    const methodWeights = data.stage === "pre-revenue" || data.stage === "seed"
  ...
  `).join("");
    const comparables = (data.detailedAnalysis?.comparableCompanies || []).map(c => `<li style="padding:6px 0;font-size:12px;color:#475569;border-bottom:1px solid #f1f5f9;">${c}</li>`).join("");
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
