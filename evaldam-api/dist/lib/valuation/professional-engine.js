...
exports.ProfessionalValuationEngine = void 0;
const scorecard_1 = require("@/lib/claude/methods/scorecard");
const berkus_1 = require("@/lib/claude/methods/berkus");
const vcMethod_1 = require("@/lib/claude/methods/vcMethod");
const dcfLTG_1 = require("@/lib/claude/methods/dcfLTG");
const dcfMultiples_1 = require("@/lib/claude/methods/dcfMultiples");
const evaldam_score_1 = require("@/lib/claude/methods/evaldam-score");
...
class ProfessionalValuationEngine {
     ...
     */
    async execute() {
        logger_1.logger.info("Evaldam: Professional Valuation Started", {
        ...
        const results = await Promise.allSettled([
            new scorecard_1.ScorecardMethod(this.profile).execute(),
            new berkus_1.BerkusMethod(this.profile).execute(),
            new vcMethod_1.VCMethod(this.profile).execute(),
            new dcfLTG_1.DCFLTGMethod(this.profile).execute(),
            new dcfMultiples_1.DCFMultiplesMethod(this.profile).execute(),
            new evaldam_score_1.EvalDamScoreMethod(this.profile).execute(),
        ]);
        ...
        results.forEach((result, index) => {
            if (result.status === 'fulfilled') {
                ...
                logger_1.logger.info(`${methodNames[index]} succeeded`, { valuation: result.value.midEstimate });
            }
            else {
                logger_1.logger.warn(`${methodNames[index]} failed, using fallback`, { error: result.reason instanceof Error ? result.reason.message : String(result.reason) });
        ...
        const result = {
            ...
            executiveSummary: {
                ...
                keyReasons: this.generateKeyReasons(methods, blendedValuation),
                methodologyNote: "Valuation derived from 5 professional methods with stage-weighted blending. See detailed analysis for full calculations and sources.",
        ...
        logger_1.logger.info("EQUIDAM: Valuation Complete", {
    ...
    identifyComparables() {
        ...
        return (comparablesByIndustry[industry] || [
...
Sources: Federal Reserve, Damodaran 2026 Cost of Equity tables, PitchBook, S&P CapitalIQ`;
     ...
     */
    calculateDynamicWeights(methods) {
        const stage = this.profile.stage;
     ...
     */
    blendValuations(methods, weights) {
        let weightedMid = 0;
        let allLows = [];
        let allHighs = [];
     ...
     */
    createMethodBreakdown(methods, weights) {
        const breakdown = {};
        for (const method of methods) {
            const weight = weights[method.methodName] || 1 / methods.length;
     ...
     */
    generateSensitivityAnalysis(baseValuation) {
     ...
     */
    generateKeyReasons(methods, blendedValuation) {
        const reasons = [];
        const arr = this.profile.annualRecurringRevenue || 0;
        ...
        // Reason 1: Traction
        if (growth > 15) {
            reasons.push(`Strong monthly growth of ${growth.toFixed(1)}% demonstrates rapid scaling and product-market fit validation`);
        }
        else if (arr > 1000000) {
            reasons.push(`Existing revenue of $${(arr / 1000000).toFixed(1)}M ARR provides strong traction and unit economics validation`);
        }
        else {
        ...
        if (this.profile.accelerators?.some((a) => a.name === "YC" || a.name === "Techstars")) {
            reasons.push(`Backed by top-tier accelerator (YC/Techstars) — signals quality and network access`);
        }
        ...
        if (this.profile.totalAddressableMarket && this.profile.totalAddressableMarket > 1000000000) {
            reasons.push(`Large addressable market ($${(this.profile.totalAddressableMarket / 1000000000).toFixed(1)}B+) with multiple expansion runway`);
        }
        // Reason 4: Method convergence
        const methodSpread = Math.max(...methods.map((m) => m.highEstimate)) /
            Math.min(...methods.map((m) => m.lowEstimate));
        if (methodSpread < 3) {
            reasons.push(`Multiple valuation methods converge ($${blendedValuation.low.toLocaleString()}–$${blendedValuation.high.toLocaleString()}), indicating consistent valuation signal`);
        }
     ...
     */
    determineConfidenceLevel(methods) {
...
exports.ProfessionalValuationEngine = ProfessionalValuationEngine;
//# sourceMappingURL=professional-engine.js.map