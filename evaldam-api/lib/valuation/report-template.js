...
function generateProfessionalReport(result, profile) {
    const report = `
...
|----------|----------|--------|----------|
${result.sensitivityAnalysis.map(s => `| ${s.variable} | ${s.scenario} | $${s.impact >= 0 ? '+' : ''}${s.impact.toLocaleString()} | ${s.percentageChange >= 0 ? '+' : ''}${s.percentageChange}% |`).join('\n')}
