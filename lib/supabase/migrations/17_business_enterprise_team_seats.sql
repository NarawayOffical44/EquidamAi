-- Align team-seat entitlements with the paid plan model:
-- Startup = solo owner, Agency / Investor = 5 team members, Enterprise = unlimited.
...
COMMENT ON FUNCTION check_team_seats_limit(UUID) IS
  'Returns used and available member seats. Agency / Investor plans get 5 member seats; Enterprise gets unlimited seats. Startup is solo owner only.';
